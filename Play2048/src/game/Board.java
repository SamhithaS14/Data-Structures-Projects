...
public class Board {
    ...
    public void addRandomTile() {
        ...
        gameBoard[openSpaces.get(t).getRow()][openSpaces.get(t).getCol()] = v;
    }
     ...
     **/
    public void rotateBoard() { //calls transpose and flipRows in sequence to complete a 90 degree clockwise rotation
        transpose();
        flipRows();
    }
    ...
    public void transpose() {
        ...
        for ( int i = 0; i < gameBoard.length; i++ ) {
            for ( int j = 0; j < gameBoard[i].length; j++ ) {
                transposedArray[j][i] = gameBoard[i][j];
            }
        }

         for ( int i = 0; i < transposedArray.length; i++ ) {
            for ( int j = 0; j < transposedArray[i].length; j++ ) {
                gameBoard[i][j] = transposedArray[i][j];
            }
        }
    }
    ...
    public void flipRows() {
        ...
        int[][] flipRowsArray = new int[4][4];
       
        for ( int i = 0; i < gameBoard.length; i++ ) {
            int c = 0;
            for ( int j = gameBoard[i].length - 1; j >= 0; j--) { //take values of gameBoard by row, but going from right to left
                flipRowsArray[i][c] = gameBoard[i][j];
                c++;

            } 
        }

        for ( int i = 0; i < flipRowsArray.length; i++ ) {
            for ( int j = 0; j < flipRowsArray[i].length; j++ ) {
                gameBoard[i][j] = flipRowsArray[i][j];
            }
        }
    }
