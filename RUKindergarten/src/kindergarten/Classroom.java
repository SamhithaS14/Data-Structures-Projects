...
public class Classroom {
    ...
    public void enterClassroom ( String filename ) {
        ...
        StdIn.setFile(filename); // read the file name

        int numberOfStudents = StdIn.readInt();

        for (int i = 0; i < numberOfStudents; i++){
            String firstName = StdIn.readString(); // read a student from the file by assigning variables to different attributes
            String lastName = StdIn.readString();
            int height = StdIn.readInt();

            Student student = new Student(firstName,lastName, height); //student object with the read attributes

            SNode studentNode = new SNode(student, null); //new Node object with data from Student, and a null memory address

            studentNode.setNext(studentsInLine); //insert new studentNode at the front of studentsInLine linked list
            studentsInLine = studentNode; //update the new front/studentsInLine
    ...
    public void setupSeats(String seatingChart) {
        ...
        for (int i = 0; i < studentsSitting.length; i++){
            for(int j = 0; j < studentsSitting[i].length; j++){
                seatingLocation[i][j] = StdIn.readBoolean(); //reading value from studentsSitting and put it in the same position in seatingLocation
            }
        }
    ...
    public void seatStudents () {
        ...
        for(int i = 0; i < seatingLocation.length; i++){ 
            for (int j = 0; j < seatingLocation[i].length; j++){
                if(seatingLocation[i][j] == true && studentsInLine != null && studentsSitting[i][j]==null){ 
                    studentsSitting[i][j] = studentsInLine.getStudent(); //getting the first student from the LL, and put in that array position
                    if(studentsInLine.getNext() != null){
                        studentsInLine = studentsInLine.getNext(); //updating front of LL by removing the first student that you insert into the array
                    }
                    else if(studentsInLine != null && studentsInLine.getNext() == null){
    ...
    public void insertMusicalChairs () {
        ...
        for(int i = 0; i < studentsSitting.length; i++){ 
            for(int j = 0; j < studentsSitting[0].length; j++){ 
                 ...
                 if(studentsSitting[i][j] != null){//see if there's a student there

                    if(musicalChairs == null){ //if empty linked list
                        SNode mc = new SNode(studentsSitting[i][j], null); 
                        musicalChairs = mc; //new node set to musicalChairs
                        musicalChairs.setNext(mc); //pointing it to itself
                        studentsSitting[i][j] = null;
                    } else {
                        SNode mc = new SNode(studentsSitting[i][j], musicalChairs.getNext()); 
                        musicalChairs.setNext(mc); //link to prev musicalChairs.getNext();
                        musicalChairs = mc; //create link between
                        studentsSitting[i][j] = null;
                    }
                }
            } 
    ...
    public void moveStudentFromChairsToLine(int size) {
        ...
        int n = StdRandom.uniform(size); //get random position of student [0 to 16)
        
        SNode student;
        SNode ptr = musicalChairs;

        
        if(n == 0){ //to elimate 1st
            ...
            for (int i = 0; i < n; i++){ //get to the node b4 student you want to delete
                ptr = ptr.getNext(); 
            }
            ...
            student = ptr.getNext(); //student to eliminate is next
            ptr.setNext(ptr.getNext().getNext()); //link old to one after elimination

        }
        Student studentToInsert = student.getStudent();
    ...
    public void insertByHeight(Student studentToInsert) {
        ...
        if(studentsInLine == null || studentToInsert.getHeight() < studentsInLine.getStudent().getHeight()){
            ...
            SNode ptr = studentsInLine; //prev -- will become where studentToInsert needs to be
            SNode ptr2 = studentsInLine.getNext(); //next -- need both so you can link the student to the next
            while (ptr2 != null){
                if(studentToInsert.getHeight() < ptr2.getStudent().getHeight()){
                    SNode student = new SNode(studentToInsert, ptr2);
                    ptr.setNext(student);
                    return;
                }
